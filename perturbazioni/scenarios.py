...
def generate_scenario_batches(
    ...
    n_extra_arcs, mean_frac, sigma_frac, base_seed,
    root, env, p, C,
    n_scenarios, batch_size,
):
    """
    Genera n_scenarios scenari e li divide in batch da batch_size ciascuno.
    """
    n_batches = n_scenarios // batch_size

    all_ids = list(range(1, n_scenarios + 1))
    print(f"\nGenerazione {n_scenarios} scenari "
          f"→ {n_batches} batch da {batch_size} scenari")
    ...
    for b in range(n_batches):
        ...
        batch_probs   = {sid: 1.0 / batch_size for sid in batch_ids}
