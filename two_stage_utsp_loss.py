...
def compute_heatmap(T_omega):
    ...
    # Questo è il fix — l'hai aggiunto?
    mask = torch.eye(n, dtype=torch.bool, device=T_omega.device).unsqueeze(0)
...
def _loss_asymmetry(H_list, scenario_probs):
    ...
    for H_omega, p_w in zip(H_list, scenario_probs):
        ...
        n = H_omega.size(-1)
        off_diag = (~torch.eye(n, dtype=torch.bool, device=H_omega.device)).unsqueeze(0)

        reciprocal = H_omega * H_T
        reciprocal = reciprocal.masked_fill(~off_diag, 0.0)
...
def two_stage_utsp_loss( T_list, dist_list, I_mask,
    ...
    # questo era l interruttore per la penalità, al momento è attiva perche è in loss=( qua sopra esplicita
