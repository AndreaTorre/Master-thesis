...
#Grafici andament loss 
def _plot_total_training_loss(history, exp_name="espB_UTSP"):
    """
    Salva il grafico della sola loss totale del training.
    Usa history["loss"], già riempita in _train_utsp_2stage.
    """
    if history is None or "loss" not in history or len(history["loss"]) == 0:
        print("  [plot loss] history['loss'] vuota: grafico non generato.")
        return None
    ...
    ax.plot(epochs, losses, linewidth=2, label="Total loss")
    ax.scatter([best_epoch], [best_loss], s=45, label=f"Min: ep {best_epoch}")

    ax.set_xlabel("Epoca")
    ax.set_ylabel("Loss totale")
    ax.set_title("Andamento della loss totale")
    ax.grid(True, alpha=0.3)
    ax.legend()

    fname = os.path.join(OUTPUT_DIR, f"{exp_name}_training_total_loss.png")
    fig.tight_layout()
    fig.savefig(fname, dpi=300, bbox_inches="tight")
    plt.close(fig)

    print(f"  → Grafico loss totale salvato: {fname}")
    return fname

# Addestro la GNN 
def _train_utsp_2stage(
    nodes, coords, scenario_batches,
    I_mask, p_mat, C_mat, device
):
    n = len(nodes)

    # Precalcola tensori per ogni batch fuori dal loop epoche
    batch_tensors = []
    for batch in scenario_batches:
        sids    = batch["scenario_ids"]
        results = batch["results"]
        xy_tile, dist_raw, dist_model, dist_scale, temperature = _build_input_tensors(
            sids, results, nodes, coords, device
        )
        probs_t = torch.tensor(
            [batch["scenario_probs"][sid] for sid in sids],
            dtype=torch.float32, device=device
        )
        adj_stack = torch.exp(-dist_model / temperature)
    ...
    # dist_scale e temperature dal primo batch (stesso TSP base per tutti)
    dist_scale  = batch_tensors[0]["dist_model"]
    _, _, _, dist_scale, temperature = _build_input_tensors(
        scenario_batches[0]["scenario_ids"],
        scenario_batches[0]["results"],
        nodes, coords, device
    )
    ...
    K_batch = batch_tensors[0]["K"]
    print(f"\n  Training UTSP 2-stage | device={device} | n_params={n_par:,}")
    print(f"  GNN({n}→{UTSP2_HIDDEN}×{UTSP2_NLAYERS}) | "
          f"{len(batch_tensors)} batch x {K_batch} scenari | "
          f"T={temperature:.4f} | scale={dist_scale:.4f}")
    ...
    for epoch in range(1, UTSP2_EPOCHS + 1):
        ...
        for bt in batch_tensors:
            T_batch = model(bt["xy_tile"], bt["adj_stack"], device)  # (K, n, n)
            T_list    = [T_batch[k:k+1] for k in range(bt["K"])]
            dist_list = [bt["dist_model"][k:k+1] for k in range(bt["K"])]

            loss, comps = two_stage_utsp_loss(
                T_list, dist_list, I_mask, p_mat, C_mat, bt["probs_t"],
            ...
            epoch_comps = comps  # log dell'ultimo batch

        scheduler.step()  # un solo step per epoca

        epoch_loss_mean = epoch_loss / max(len(batch_tensors), 1)
        ...
        if epoch % UTSP2_LOG_FREQ == 0 or epoch == 1:
            marker = " ★" if abs(epoch_loss_mean - best_loss) < 1e-9 else ""
            print(f"  {format_loss_components(epoch_comps, epoch)}"
                  f"  [tot={epoch_loss_mean:.4f}]{marker}")
    ...
    # Ricostruisci tensori completi su tutti gli scenari per la decode
    full_adj_stack = torch.cat([bt["adj_stack"]  for bt in batch_tensors], dim=0)
    full_dist_model = torch.cat([bt["dist_model"] for bt in batch_tensors], dim=0)
    full_xy_tile = batch_tensors[0]["xy_tile"].repeat(len(batch_tensors), 1, 1)
    full_probs_t   = torch.cat([bt["probs_t"] for bt in batch_tensors], dim=0)
    full_probs_t = full_probs_t / full_probs_t.sum().clamp(min=1e-9)
    return (model, history, full_adj_stack, full_dist_model,
            full_xy_tile, full_probs_t, temperature, dist_scale)
...
def _evaluate_policy_oos(
    ...
    results_val, _, _ = generate_scenarios(
        ...
        root=root, env=env, p=p, C=C,coords=coords, wind=wind, alpha=alpha,
    )
...
def run_esperimento_B_UTSP(nodes, coords, base_dist, E, root, env, res_B, wind, alpha, mode="local search"):
    """
    mode:
    ...
    STO_train = res_B["STO"]
    EEV_train = res_B["EEV"]
    from config import ERA5_NC_PATH, WIND_ALPHA
    wind  = load_wind_field(ERA5_NC_PATH)
    alpha = WIND_ALPHA
    # Scenari grandi usati solo per addestrare UTSP.
    # Scenari grandi divisi in batch per addestrare UTSP.
    scenario_batches = generate_scenario_batches(
    nodes, E, base_dist, I, frequent_arcs,
    N_EXTRA_ARCS, MEAN_FRAC, SIGMA_FRAC,
    UTSP_TRAINING_SEED,
    root=root, env=env, p=p, C=C,
    n_training_scenarios=N_TRAINING_SCENARIOS_UTSP,
    n_validation_scenarios=N_VALIDATION_SCENARIOS,
    coords=coords, wind=wind, alpha=alpha,
    )
    
    scenario_ids_utsp = [sid for b in scenario_batches for sid in b["scenario_ids"]]
    results_utsp      = {sid: b["results"][sid]
                         for b in scenario_batches for sid in b["scenario_ids"]}
    ...
    _plot_total_training_loss(history, exp_name="espB_UTSP")
